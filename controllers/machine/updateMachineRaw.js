...
module.exports = async (req, res) => {
    try {
        ...
        let machineData = await MachineData.findOne({ where: { machineID } });

        if (machineData) {
            await MachineData.update(updateData, { where: { machineID } });
            machineData = await MachineData.findOne({ where: { machineID } });
            await Machines.update({ lastUpdate: Math.floor(Date.now() / 1000) }, { where: { machineID } });
            return res.status(200).json({ message: 'Machine data updated.', payload: { updatedMachine: machineData } });
        } else {
